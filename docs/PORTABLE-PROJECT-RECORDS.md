...
An opt-in profile for the Check-in Desk, lightweight Work Board, and bindings to a project's
chosen tracker. This profile and its file support are development additions pending release;
the Work Board remains a pilot. Qualification is specific to the tested tools and scope.
...
Use appropriate documented formats: narrative records can remain Markdown; structured Desk
and Work records use their shared schemas in file mode. A record's identity includes its
project scope. Renames and moves preserve that identity or a durable cross-reference.

## Two tracking modes

- **File mode:** use Velocity's optional lightweight tracker. Its minimum work record is an
  ID, title, owner, and state. Additional data is required only as appropriate to the claim:
  bounded scope for active work, a blocker for blocked work, evidence for completion, a reason
  for dropping it. Dates, size, benefit, initiatives, and controls are used when applicable.
...
The file implementation uses the [Work Board](../templates/work-board.md) states. An external
tracker may use different labels if its mapping preserves these distinctions and the proof.

The Work Board describes work; the [Check-in Desk](../templates/executive-checkin-desk.md)
records operator asks and decisions under its existing contract. Work links to the relevant
Desk ask. A ruling remains in its decision ledger rather than becoming two editable approvals.
...
3. Validate with the pinned shared implementation or qualified tracker binding. Serialize
   cooperating writers or use conditional writes. A stale writer rereads and reconciles.
...
5. Read back the source and regenerate/refresh the operator view in the same turn. Identify
...
The baseline display is standalone HTML in an ordinary browser. AI side panels and hosted
pages are optional views. Shared renderers own validation, content, totals, and layout;
...
its documented limitations remain part of the qualification claim.

## Adoption and tracker migration

Import a TODO file with its original wording/provenance and a dated source copy. Preserve
unknown ownership, dates, and status as uncertainty for owner resolution. Historical reported
completion remains distinct from evidenced completion. Compare the import before retiring
the original active queue; do not invent tasks merely to initialize an empty board.
...
- an empty start and provenance-preserving TODO adoption;
